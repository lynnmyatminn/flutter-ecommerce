...
import 'package:flutter_ecommerce_app/models/JTCProductDetails.dart';
import 'package:flutter_ecommerce_app/utils/Urls.dart';
import 'package:http/http.dart';

JTCProductDetails productDetails;

class JTCProductDetailScreen extends StatefulWidget {
  String slug;

  JTCProductDetailScreen({Key key, @required this.slug}) : super(key: key);

  @override
  _JTCProductDetailScreenState createState() => _JTCProductDetailScreenState();
}
...
class _JTCProductDetailScreenState extends State<JTCProductDetailScreen> {
  @override
...
Widget createDetailView(BuildContext context, AsyncSnapshot snapshot) {
  JTCProductDetails values = snapshot.data;
  return DetailScreen(
...
Future<JTCProductDetails> getDetailData(String slug) async {
  Response response;
  response = await get(Urls.JTC_URL + slug);
  int statusCode = response.statusCode;
  final body = json.decode(response.body);
  if (statusCode == 200) {
    productDetails = JTCProductDetails.fromJson(body);
    return productDetails;
  } else {
    return productDetails = JTCProductDetails();
  }
}
